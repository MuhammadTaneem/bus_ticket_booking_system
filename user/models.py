...
class Usermanager(BaseUserManager):
    def create_user(self, email, user_type, phone_number, password=None, **extra_fields):
        ...
        user = self.model(email=email, user_type=user_type, phone_number=phone_number, **extra_fields)
        user.set_password(password)
        user.save(using=self._db)
        return user

    def create_superuser(self, email, user_type, phone_number, password=None, **extra_fields):
